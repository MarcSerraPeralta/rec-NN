...
class model(torch.nn.Module):
	def __init__(self, **kwargs):
		...
		Scheme
		------
		 INP     Embed+ReLU    H1   Linear+ReLU     H2   Linear+ReLU     H3   Linear    OUTOUT
		"10^6" ------------> emdim -------------> emdim- ------------> emdim ---------> "10^6"
		"""
		...
		self.w2 = torch.nn.Linear(self.dim[1], self.dim[0])
	...
	def encoder(self, x):
		...
		if self.reduction_emb == "attention":
			...
			h = (att*h).sum(dim=1)
			h = self.relu(h)
		z = self.relu(self.w1(h))
		return z, None

	def decoder(self, x):
		h = self.relu(self.w2(x))
		xhat = self.inv(h)
		return xhat
