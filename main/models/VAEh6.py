...
class model(torch.nn.Module):
	def __init__(self, **kwargs):
		...
		ENCODER																	Linear
													  						   |-------> mu 	(dim[3])
		 INP     Embed+ReLU    H1   Linear+ReLU     H2   Linear+ReLU     H3    |
		"10^6" ------------> dim[0] ------------> dim[1] ------------> dim[2] -|
														  					   | Linear
																			   |-------> logvar	(dim[3])
		...
		DECODER
		   ...
		   z    Linear+ReLU     H4     Linear+ReLU     H5     Linear+ReLU     H6     Linear    OUTOUT
		dim[3] -------------> dim[2] --------------> dim[1] --------------> dim[0] ----------> "10^6"
		...
		self.mu = torch.nn.Linear(self.dim[2], self.dim[3])
		self.mu.bias.data.fill_(0)
		self.logvar = torch.nn.Linear(self.dim[2], self.dim[3])
