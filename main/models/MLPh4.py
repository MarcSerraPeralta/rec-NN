...
class model(torch.nn.Module):
	def __init__(self, **kwargs):
		...
		Scheme
		------
		 INP     Embed+ReLU    H1   Linear+ReLU     H2   Linear+ReLU     H3   Linear+ReLU     H4   Linear+ReLU     H5   Linear    OUTOUT
		"10^6" ------------> emdim -------------> emdim- ------------> emdim  ------------> emdim  ------------> emdim ---------> "10^6"
		"""
	...
	def encoder(self, x):
		...
		z = self.relu(self.w2(h))
		return z, None

	def decoder(self, x):
		h = self.relu(self.w3(x))
